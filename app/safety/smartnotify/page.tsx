...
export default function SmartNotify() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4"><Card className="p-4"><p className="text-xs text-ink-500">Affected customers</p><p className="mt-1 text-2xl font-bold text-ink-900">{zone.n.toLocaleString("en-IN")}</p><p className="text-xs text-ink-500">Domestic 91% · Commercial 9%</p></Card><Card className="p-4"><p className="text-xs text-ink-500">Delivery coverage</p><p className="mt-1 text-2xl font-bold text-brand-700">95.6%</p><p className="text-xs text-ink-500">1,228 delivered · 56 fallback</p></Card><Card className="p-4"><p className="text-xs text-ink-500">Predicted support calls</p><p className="mt-1 text-2xl font-bold text-ink-900">82</p><p className="text-xs text-ink-500">Commercial impact: medium</p></Card><Card className="p-4"><p className="text-xs text-ink-500">Compliance</p><p className={`mt-1 text-lg font-bold ${start && !compliant ? "text-red-600" : "text-brand-700"}`}>{start && !compliant ? "At risk" : "Ready"}</p><p className="text-xs text-ink-500">48-hour notice requirement</p></Card></div>
              ...
              <ul className="space-y-2 max-h-72 overflow-y-auto">
                {resp.results.map((r, i) => (
                  <li key={i} className="flex items-center gap-2 text-sm animate-slideIn">
                    ...
                    <span className={`ml-auto text-xs ${r.status === "sent" ? "text-brand-600" : "text-red-500"}`}>
              ...
              <ul className="space-y-2 text-sm">
                {sim.log.map((n, i) => (
                  <li key={i} className="flex items-center gap-2 text-ink-600 animate-slideIn">
                    ...
                    <span className="text-xs text-ink-400 ml-auto">delivered</span>
                  </li>
                ))}
              </ul>
            </div>
