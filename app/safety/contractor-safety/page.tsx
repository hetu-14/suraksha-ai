      ...
      <div className="grid lg:grid-cols-3 gap-6 anim-fade-up">
        ...
        <Card className="lg:col-span-2 overflow-hidden">
          ...
          <div className="p-4 sm:p-5">
            <DataTable
              columns={[
                { key: "name", header: "Contractor Name", primary: true, cell: (c) => <span className="font-bold text-ink-800">{c.name}</span> },
                { key: "incidents", header: "Incidents", secondary: true, cell: (c) => <span>{c.incidents} incidents MTD</span> },
                {
                  key: "score", header: "Safety Index", align: "right",
                  cell: (c) => <span className={`font-bold tabular-nums ${c.score >= 85 ? "text-brand-600" : c.score >= 70 ? "text-amber-600" : "text-red-600"}`}>{c.score}</span>,
                },
                {
                  key: "rating", header: "Compliance", align: "right",
                  cell: (c) => <Badge tone={c.rating === "Excellent" ? "brand" : c.rating === "Satisfactory" ? "sky" : c.rating === "Needs Audit" ? "amber" : "red"}>{c.rating}</Badge>,
                },
              ]}
              rows={contractors}
              getKey={(c) => c.name}
              onRowClick={(c) => setSel(contractors.findIndex((item) => item.name === c.name))}
              isActive={(c) => contractors[sel]?.name === c.name}
              caption="Contractor safety scoreboard"
            />
          </div>
        </Card>
        ...
        <div className="space-y-4">
          ...
          <Card className="p-5 bg-amber-50/50 border-amber-500/10 text-xs">
