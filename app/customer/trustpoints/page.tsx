...
import {
  ...
  UsersRound, Wrench, Zap, SearchCheck, ScrollText, Briefcase, CreditCard, Link2, Tag, RadioTower,
} from "lucide-react";
...
const rewards = [
  { id: "annual", title: "Free annual inspection", points: 400, group: "Safety rewards", icon: ShieldCheck },
  { id: "leak", title: "Free leak testing", points: 300, group: "Safety rewards", icon: SearchCheck },
  { id: "certificate", title: "Safety awareness certificate", points: 150, group: "Safety rewards", icon: ScrollText },
  { id: "kit", title: "Home emergency kit", points: 600, group: "Safety rewards", icon: Briefcase },
  { id: "credit", title: "Bill credit ₹100", points: 300, group: "Service rewards", icon: CreditCard },
  { id: "reconnect", title: "Free reconnection fee", points: 450, group: "Service rewards", icon: Link2 },
  { id: "appliance", title: "PNG appliance discount", points: 350, group: "Partner rewards", icon: Tag },
  { id: "meter", title: "Smart meter upgrade discount", points: 1000, group: "Partner rewards", icon: RadioTower },
];
...
export default function TrustPoints() {
  ...
  return <div className="space-y-6 reveal">
    <header className="rounded-xl bg-ink-950 text-white p-6 sm:p-8 relative overflow-hidden "><div className="relative max-w-3xl"><p className="text-brand-300 text-xs font-semibold uppercase tracking-widest">Safety & engagement rewards</p><h1 className="text-fluid-h1 font-bold mt-1">TrustPoints for safer gas living.</h1><p className="text-ink-300 mt-2 text-sm">Earn rewards for safer, more responsible PNG use—timely payments, inspections, training, and keeping your safety details current.</p></div></header>
    <nav aria-label="TrustPoints sections" className="sticky top-3 z-10 flex gap-1 scroll-x rounded-xl border border-ink-100 bg-white/95 p-1.5  backdrop-blur">{([ ["overview", "Overview"], ["missions", "Safety missions"], ["rewards", "Rewards"], ["history", "Points history"] ] as const).map(([id, label]) => <button key={id} onClick={() => setTab(id)} className={`shrink-0 rounded-xl px-3.5 py-2 text-sm font-semibold transition ${tab === id ? "bg-ink-900 text-white" : "text-ink-500 hover:bg-ink-50 hover:text-ink-900"}`}>{label}</button>)}</nav>
    ...
    {tab === "overview" && <><div className="grid grid-cols-2 lg:grid-cols-4 gap-4"><Kpi label="TrustPoints balance" value={points.toLocaleString("en-IN")} sub="Available for safety & service rewards" icon={<Star className="w-4 h-4" />} /><Kpi label="Safety reputation" value={tier.name} sub="Responsible customer rating" icon={<ShieldCheck className="w-4 h-4 text-brand-500" />} /><Kpi label={nextTier ? `Points to ${nextTier.name}` : "Top reputation"} value={nextTier ? away : "Achieved"} sub={nextTier ? "Next reputation milestone" : "Thank you for leading safely"} icon={<TrendingUp className="w-4 h-4" />} /><Kpi label="Earned this month" value={profile.ledger.filter((row) => row.date === "Today").reduce((sum, row) => sum + Math.max(0, row.points), 0)} sub="Safety actions completed" icon={<Zap className="w-4 h-4 text-brand-500" />} /></div>
      <div className="grid lg:grid-cols-3 gap-5"><Card className="lg:col-span-2 p-5"><div className="flex items-start justify-between gap-3"><div><h2 className="font-bold text-ink-900">Safety Guardian journey</h2><p className="text-sm text-ink-600 mt-1">{points.toLocaleString("en-IN")} points earned. {nextTier ? `${away} points away from ${nextTier.name}.` : "You have reached the highest safety reputation."}</p></div><span className="h-11 w-11 rounded-xl bg-brand-100 text-brand-700 grid place-items-center"><tier.icon className="w-5 h-5" /></span></div><div className="mt-5"><div className="flex justify-between text-xs font-semibold"><span>{tier.name}</span><span>{nextTier?.name ?? "Community Protector"}</span></div><div className="h-3 bg-ink-100 rounded-full overflow-hidden mt-2"><div className="h-full bg-brand-500 rounded-full transition-all" style={{ width: `${progress}%` }} /></div><div className="flex justify-between text-xs text-ink-400 mt-1"><span>{tier.threshold.toLocaleString("en-IN")} points</span><span>{nextTier?.threshold.toLocaleString("en-IN") ?? points.toLocaleString("en-IN")} points</span></div></div><div className="grid sm:grid-cols-3 gap-3 mt-5">{missions.slice(0, 3).map((mission) => <div key={mission.id} className="rounded-xl bg-ink-50 p-3"><div className="text-xs text-ink-500">Ways to earn</div><div className="font-semibold text-sm text-ink-800 mt-1">{mission.title}</div><div className="font-bold text-brand-700 mt-2">+{mission.reward}</div></div>)}</div></Card><Card className="p-5"><h2 className="font-bold text-ink-900 flex items-center gap-2"><Award className="w-4 h-4 text-brand-600" /> Safety reputation</h2><div className="text-2xl font-bold text-brand-700 mt-4">Excellent</div><ul className="mt-4 space-y-2 text-xs text-ink-700">{["3 years without incidents", "Timely safety inspections", "No overdue bills", "Emergency details maintained"].map((reason) => <li key={reason}><Check className="w-3.5 h-3.5 inline mr-1 text-brand-600" />{reason}</li>)}</ul><div className="mt-4 rounded-xl bg-brand-50 p-3 text-xs text-brand-800"><UsersRound className="w-3.5 h-3.5 inline mr-1" /> Area average: <strong>Safe User</strong> · You are above average</div><Link href="/customer/health" className="mt-4 inline-flex items-center gap-1 text-xs font-bold text-brand-700 hover:underline">View your full health score breakdown <ChevronRight className="w-3.5 h-3.5" /></Link></Card></div>
