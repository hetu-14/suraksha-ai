  ...
  return (
    <div className="space-y-6 reveal">
      <header className="rounded-xl bg-ink-950 text-white p-6 sm:p-8 relative overflow-hidden ">
        <div className="relative max-w-3xl"><p className="text-brand-300 text-xs font-semibold uppercase tracking-widest">SuRaksha AI · Customer safety</p><h1 className="text-2xl sm:text-3xl font-bold mt-1">Gas Safety & Service Health Index</h1><p className="text-ink-300 mt-2 text-sm">A simple view of whether your connection is safe, equipment is healthy, payments are on track, and any action needs your attention.</p></div>
      </header>

      <nav aria-label="Health score sections" className="sticky top-3 z-10 flex gap-1 overflow-x-auto rounded-xl border border-ink-100 bg-white/95 p-1.5  backdrop-blur">
...
function ScoreStep({ label, value }: { label: string; value: string }) {
  return <div className="rounded-xl bg-ink-50 p-3"><p className="text-[11px] text-ink-500">{label}</p><p className="mt-1 text-lg font-bold text-ink-900">{value}</p></div>;
}
