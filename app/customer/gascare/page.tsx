...
export default function GasCarePage() {
  ...
  return <div className={`space-y-6 reveal ${active ? "pb-[calc(5.5rem+env(safe-area-inset-bottom))]" : ""}`}>
    {/* While a session is live the emergency call bar owns the thumb zone, so
        the suite's bottom tab bar stands down (see globals.css crisis rules). */}
    {active && <CrisisCallBar />}
    {notice && <div className="fixed right-4 top-4 z-50 flex max-w-sm items-start gap-3 rounded-xl bg-brand-600 px-5 py-3 text-white shadow-xl"><CheckCircle2 className="h-5 w-5 shrink-0" /><span className="flex-1 text-sm font-semibold">{notice}</span><button onClick={() => setNotice(null)} aria-label="Dismiss message"><XCircle className="h-4 w-4" /></button></div>}
    ...
    {!active ? <>
      <header className="relative overflow-hidden rounded-xl border border-red-100 bg-ink-950 p-6 text-white  sm:p-8">
        <div className="relative grid gap-6 lg:grid-cols-[1.25fr_.75fr] lg:items-center">
          <div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-red-300">GasGuard · household emergency protection</p><h1 className="mt-2 text-fluid-h1 font-bold">Be ready before an emergency.</h1><p className="mt-3 max-w-2xl text-sm leading-relaxed text-ink-300">Build your household safety plan, practise a guided drill, and get type-specific emergency help when it matters.</p></div>
          <div className="rounded-xl border border-white/10 bg-white/10 p-4 backdrop-blur"><div className="flex items-center justify-between"><span className="text-sm font-semibold">Emergency readiness</span><span className="text-2xl font-bold text-brand-300">{readiness}%</span></div><div className="mt-3 h-2 rounded-full bg-white/15"><div className="h-full rounded-full bg-brand-400" style={{ width: `${readiness}%` }} /></div><p className="mt-3 text-xs text-ink-300">{profile.emergencyContactVerified ? "Your household profile is ready for emergency outreach." : "Verify an emergency contact to complete your plan."}</p><p className="mt-3 flex items-center gap-1.5 text-xs font-semibold text-brand-200"><WifiOff className="h-3.5 w-3.5" /> Offline safety guide saved on this device</p></div>
        </div>
      </header>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="p-5 lg:col-span-2"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="font-bold text-ink-900">Are you experiencing?</h2><p className="mt-1 text-xs text-ink-500">Choose the closest situation so GasGuard can guide and prioritize the right response.</p></div><Badge tone={selectedType.risk === "Critical" ? "red" : "amber"}>{selectedType.risk} risk route</Badge></div><div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{emergencyTypes.map((item) => <button key={item.id} onClick={() => setType(item.id)} className={`min-h-crisis rounded-xl border p-4 text-left transition ${type === item.id ? toneClass(item.tone) : "border-ink-100 bg-white hover:border-red-200 hover:bg-red-50/40"}`}><item.icon className="h-5 w-5" /><p className="mt-3 text-sm font-bold">{item.label}</p><p className="mt-1 text-xs opacity-75">{item.detail}</p></button>)}</div><button onClick={() => setConfirmOpen(true)} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-red-600 px-5 py-3.5 text-sm font-bold text-white shadow-lg shadow-red-500/20 transition hover:bg-red-700 active:scale-[.99]"><Siren className="h-5 w-5" /> Start emergency session</button><div className="mt-4 grid grid-cols-2 gap-2 text-center sm:grid-cols-5">{["AI guide starts", "Safety steps begin", "Location captured", "Crew dispatched", "Track live"].map((item, index) => <div key={item} className="rounded-lg bg-ink-50 px-2 py-2 text-xs font-semibold text-ink-600"><span className="mr-1 text-brand-700">{index + 1}.</span>{item}</div>)}</div><a href="tel:1906" className="mt-3 flex justify-center gap-2 text-xs font-bold text-red-700 hover:underline"><PhoneCall className="h-4 w-4" /> Call gas emergency helpline · 1906</a></Card>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="p-5"><div className="flex items-center justify-between"><div><h2 className="font-bold text-ink-900">Household readiness</h2><p className="mt-1 text-xs text-ink-500">Your personal emergency safety passport.</p></div><span className="text-xl font-bold text-brand-700">{readiness}%</span></div><div className="mt-5 space-y-3">{readinessItems.map((item) => <div key={item.label} className="flex items-center gap-3"><span className={`grid h-8 w-8 place-items-center rounded-xl ${item.done ? "bg-brand-100 text-brand-700" : "bg-amber-100 text-amber-700"}`}>{item.done ? <Check className="h-4 w-4" /> : <CircleAlert className="h-4 w-4" />}</span><div className="min-w-0"><p className="text-sm font-semibold text-ink-800">{item.label}</p><p className="text-xs text-ink-500">{item.detail}</p></div>{item.label === "Emergency contact" && !item.done && <button onClick={() => persistProfile({ emergencyContactVerified: true })} className="ml-auto text-xs font-bold text-brand-700">Verify</button>}</div>)}</div></Card>
