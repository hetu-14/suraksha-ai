    ...
    <section className="relative overflow-hidden rounded-xl bg-ink-950 px-6 py-7 sm:px-8 sm:py-9 text-white ">
      ...
      <div className="relative grid gap-7 lg:grid-cols-[1.35fr_.65fr] lg:items-end">
        <div>
          ...
          <h1 className="mt-1 text-fluid-h1 font-bold">Your household is <span className="text-brand-300">safe</span></h1>
...
function Snapshot({ label, value, detail, tone }: { label: string; value: string; detail: string; tone: "brand" | "amber" }) {
  return <div className={`rounded-xl border p-4 ${tone === "brand" ? "border-brand-100 bg-brand-50/60" : "border-amber-200 bg-amber-50/60"}`}><p className="text-xs text-ink-500">{label}</p><p className={`mt-1 text-sm font-bold ${tone === "brand" ? "text-brand-800" : "text-amber-800"}`}>{value}</p><p className="mt-1 text-xs text-ink-500">{detail}</p></div>;
}
...
function QuickAction({ icon, label, detail, tone, href, onClick }: { icon: React.ReactNode; label: string; detail: string; tone: "brand" | "amber" | "sky"; href?: string; onClick?: () => void | false }) {
  ...
  const body = <><span className="grid h-9 w-9 place-items-center rounded-xl bg-white shadow-sm">{icon}</span><span className="min-w-0 flex-1"><span className="block text-xs font-bold">{label}</span><span className="mt-0.5 block truncate text-xs opacity-75">{detail}</span></span><ChevronRight className="h-4 w-4 opacity-50" /></>;
...
function Upcoming({ date, title, detail, accent }: { date: string; title: string; detail: string; accent: "brand" | "amber" | "sky" | "ink" }) {
  ...
  return <div className="relative flex gap-3 py-2.5 last:pb-0"><div className="relative flex w-14 shrink-0 flex-col items-center"><span className={`w-full rounded-lg border px-1 py-1.5 text-center text-xs font-bold ${colors[accent]}`}>{date}</span><span className="absolute -bottom-3 h-3 border-l border-dashed border-ink-200 last:hidden" /></div><div className="min-w-0 pt-0.5"><p className="text-sm font-semibold text-ink-700">{title}</p><p className="mt-0.5 text-xs text-ink-400">{detail}</p></div></div>;
}
...
function Progress({ label, value, target, suffix }: { label: string; value: number; target: number; suffix: string }) {
  ...
  return <div><div className="flex items-end justify-between gap-2"><span className="text-xs font-semibold text-ink-600">{label}</span><span className="text-xs font-bold text-ink-800">{value.toLocaleString("en-IN")}{suffix}</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-ink-100"><div className="h-full rounded-full bg-brand-500 transition-all" style={{ width: `${progress}%` }} /></div><p className="mt-1.5 text-xs text-ink-400">Target: {target.toLocaleString("en-IN")}{suffix}</p></div>;
}
