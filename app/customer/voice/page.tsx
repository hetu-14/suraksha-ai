...
export default function VoiceOfCustomer() {
  ...
  function submit(event: React.FormEvent) {
    ...
    // One emit — the effect engine routes it to the customer feed, and when
    // safety-flagged, to the control room and the recommendation engine too.
    emitPlatformEvent({ type: "ComplaintSubmitted", module: "Voice of Customer", summary: `Feedback ${item.id} submitted · ${item.category}`, entities: [{ type: "complaint", id: item.id, label: item.category }], data: { complaintId: item.id, category: item.category, priority: item.priority, owner: item.owner, safety: isSafety, text: item.text } });
    setSubmitted(item);
    setText("");
    setVoiceUrl(null);
    setTab("mine");
  ...
  function rate(itemId: string, satisfaction: Satisfaction) {
    if (satisfaction === "no") emitPlatformEvent({ type: "ComplaintReopened", module: "Voice of Customer", summary: `${itemId} reopened after failed satisfaction recheck`, entities: [{ type: "complaint", id: itemId }], data: { complaintId: itemId } });
  ...
  function escalate(kind: "review" | "callback" | "grievance") {
    emitPlatformEvent({ type: "ComplaintEscalated", module: "Voice of Customer", summary: kind === "review" ? "Feedback review requested" : kind === "callback" ? "Callback requested" : "PNGRB grievance raised", data: { kind } });
  ...
  return <div className="space-y-6 reveal">
    ...
    {tab === "mine" && <><div className="grid grid-cols-2 lg:grid-cols-4 gap-4"><Kpi label="My feedback impact" value={`${implemented} implemented`} sub="Ideas that improved service" icon={<CheckCircle2 className="w-4 h-4 text-brand-500" />} /><Kpi label="Under review" value={underReview} sub={underReview ? "Expected update within 24 hrs" : "No open issue"} icon={<Clock3 className="w-4 h-4" />} /><Kpi label="My open issues" value={underReview} sub={underReview ? "Team has been assigned" : "Nothing waiting on you"} icon={<MessageCircle className="w-4 h-4" />} /><Kpi label="Estimated reach" value="8,000+" sub="Customers helped by feedback" icon={<UsersRound className="w-4 h-4" />} /></div>
      <Card className="p-5 border-brand-100 bg-brand-50/60"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wider text-brand-700">Your voice summary</p><h2 className="mt-1 font-bold text-ink-900">Your feedback creates visible service improvements.</h2></div><span className="rounded-full bg-white px-3 py-1.5 text-xs font-bold text-brand-700 shadow-sm">Impact reach · 8,000+ customers</span></div><div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-3"><VoiceMetric label="Submitted" value={String(mine.length)} /><VoiceMetric label="Implemented" value={String(implemented)} good /><VoiceMetric label="In review" value={String(underReview)} /><VoiceMetric label="Rejected" value="0" /></div></Card>
