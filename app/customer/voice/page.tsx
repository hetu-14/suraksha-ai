...
function VoiceMetric({ label, value, good = false }: { label: string; value: string; good?: boolean }) {
  return <div className="rounded-xl border border-brand-100 bg-white p-3"><p className="text-[11px] text-ink-500">{label}</p><p className={`mt-1 text-lg font-bold ${good ? "text-brand-700" : "text-ink-900"}`}>{value}</p></div>;
}
