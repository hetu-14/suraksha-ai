...
export default function BillIntelligence({ bill, previousBill, customer, explanation, away, onAwayChange, onAsk, onDownload, usageHistory }: Props) {
  ...
  return (
    <div className="space-y-5">
      <nav aria-label="Bill intelligence sections" className="sticky top-3 z-10 flex gap-1 scroll-x rounded-xl border border-ink-100 bg-white/95 p-1.5  backdrop-blur">
        {([
      ...
      {activeTab === "overview" && <div className="space-y-5">
        ...
        <section className="rounded-xl border border-ink-100 bg-white p-5 "><div className="flex items-start gap-3"><div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-violet-50 text-violet-700"><UsersRound className="h-5 w-5" /></div><div><p className="text-xs font-bold uppercase tracking-wider text-violet-700">Impact on Torrent Gas</p><h3 className="mt-1 font-bold text-ink-900">A clear bill explanation can prevent an avoidable service call.</h3></div></div><div className="mt-4 grid gap-3 md:grid-cols-2"><ImpactPath title="Before WhyMyBill" steps={["Customer receives bill", "Calls customer care", "Raises complaint", "Engineer visit may be needed"]} tone="ink" /><ImpactPath title="After WhyMyBill" steps={["Customer receives explanation", "Understands the change", "Self-resolves the doubt", "No complaint raised"]} tone="brand" /></div></section>
      </div>}
      ...
      <section className="bg-white rounded-xl  border border-ink-100 overflow-hidden">
        ...
        <div className="divide-y divide-ink-100">
          ...
          <div className="px-5 py-4 flex items-center justify-between bg-ink-900 text-white">
            ...
            <span className="text-xl font-bold tabular-nums">{inr(bill.amount)}</span>
          </div>
        </div>
      </section>
...
function SummaryChip({ label, value, tone }: { label: string; value: string; tone: "brand" | "amber" | "ink" }) {
  ...
  return <div className={`rounded-xl border p-3 ${styles[tone]}`}><p className="text-xs font-semibold opacity-70">{label}</p><p className="mt-1 text-sm font-bold">{value}</p></div>;
}

function ImpactPath({ title, steps, tone }: { title: string; steps: string[]; tone: "brand" | "ink" }) {
  return <div className={`rounded-xl border p-4 ${tone === "brand" ? "border-brand-100 bg-brand-50/60" : "border-ink-100 bg-ink-50"}`}><p className={`text-sm font-bold ${tone === "brand" ? "text-brand-800" : "text-ink-800"}`}>{title}</p><div className="mt-3 space-y-1.5">{steps.map((step, index) => <div key={step} className="flex items-center gap-2 text-xs text-ink-700"><span className={`grid h-5 w-5 place-items-center rounded-full text-xs font-bold ${tone === "brand" ? "bg-brand-100 text-brand-700" : "bg-white text-ink-500"}`}>{index + 1}</span>{step}{index < steps.length - 1 && <ChevronDown className="ml-auto h-3.5 w-3.5 text-ink-300" />}</div>)}</div></div>;
}
...
function StoryMetric({ label, value, total }: { label: string; value: number; total: number }) {
  ...
  return <div className="rounded-xl bg-ink-50 p-3"><div className="text-xs text-ink-500">{label}</div><div className={`font-bold mt-1 tabular-nums ${value > 0 ? "text-red-600" : value < 0 ? "text-brand-700" : "text-ink-700"}`}>{sign}{inr(Math.abs(value))} <span className="text-xs text-ink-400">({contribution}%)</span></div></div>;
}
...
function TrustMeter({ score, readingVerified, chargeReconciled, manualAdjustment, patternMatched }: { score: number; readingVerified: boolean; chargeReconciled: boolean; manualAdjustment: boolean; patternMatched: boolean }) {
  ...
  return <section className="rounded-xl border border-brand-200 bg-brand-50/60 p-5 "><h3 className="font-bold text-ink-900 flex items-center gap-2"><WalletCards className="w-4 h-4 text-brand-600" /> Bill trust score</h3><div className="flex items-end gap-2 mt-3"><span className="text-4xl font-bold text-brand-700">{score}</span><span className="text-sm text-ink-500 mb-1">/ 100</span></div><div className="h-2 rounded-full bg-white overflow-hidden mt-3"><div className="h-full bg-brand-500 rounded-full" style={{ width: `${score}%` }} /></div><ul className="mt-4 space-y-1.5 text-xs text-ink-700">{checks.map(([passes, label]) => <li key={label}><Check className={`w-3.5 h-3.5 inline mr-1 ${passes ? "text-brand-600" : "text-ink-300"}`} />{label}{!passes && " · needs review"}</li>)}</ul></section>;
}
