...
export default function PlatformTimeline({ limit = 10, className = "" }: { limit?: number; className?: string }) {
  ...
  return <Card className={`p-5 ${className}`}>
    ...
    <div className="mt-3 flex gap-1.5 scroll-x pb-1">
      {FILTERS.map((category) => <button key={category} onClick={() => setFilter(category)} className={`shrink-0 rounded-full border px-2.5 py-1 text-xs font-bold transition ${filter === category ? "border-ink-900 bg-ink-900 text-white" : "border-ink-200 text-ink-500 hover:border-ink-300 hover:text-ink-800"}`}>{category}</button>)}
    </div>
    <div className="mt-3 divide-y divide-ink-100">
      ...
      {visible.map((item) => <Link key={item.id} href={item.href} className="group flex gap-3 py-3 transition hover:bg-ink-50/60">
        <span className="w-16 shrink-0 pt-0.5 text-xs tabular-nums text-ink-400">{timeAgo(item.at)}</span>
        <span className="min-w-0 flex-1">
          <span className="flex flex-wrap items-center gap-2">
            <span className={`rounded-full px-2 py-0.5 text-xs font-bold ${CATEGORY_STYLE[item.category]}`}>{item.category}</span>
            <span className="text-sm font-semibold text-ink-800 group-hover:text-ink-950">{item.title}</span>
          </span>
          <span className="mt-0.5 block text-xs text-ink-500">{item.module}{item.detail ? ` · updated ${item.detail}` : ""}</span>
          {item.entities.length > 0 && <span className="mt-1 flex flex-wrap gap-1">{item.entities.slice(0, 3).map((entity) => <span key={`${entity.type}-${entity.id}`} className="rounded border border-ink-100 bg-white px-1.5 py-0.5 text-xs font-semibold text-ink-500">{entity.label ?? entity.id}</span>)}</span>}
        </span>
      </Link>)}
    </div>
  </Card>;
